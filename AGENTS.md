...
The Throws database (env `THROWS_DB_ID`) tracks thrown work. A throw can name the combo it used via a `Glaze Combos` relation (to the Glaze Combos rows above), carry a `Clay` relation, and hold a `Photos` files property where the owner uploads stage-by-stage shots — the latest image is the finished, glazed piece. Build a combo index mapping every Glaze Combos row id (any status, so untiled combos resolve too) to its {base, top} codes via the same relation/Layers/Name resolution as tiles. A throw becomes a "finished piece" when its Status is Finished or Fired, its combo resolves, it has a Clay, and it has at least one image; emit `{base, top, clay, photo}` using the latest image. Pieces hang off the same (base, top, clay) key. A band with a piece but no finished tile renders dark grey `#5b554c` (between the finished black and skip grey) and a finished tile outranks a piece. The detail panel shows piece photos in their own "Finished pieces" section, and pieces count toward the progress bar as bands made.
