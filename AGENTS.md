...
For a Files and media property, each entry is a file (`.file.url`, a signed url that expires about an hour after the fetch, which is fine for a fresh no-store load) or external (`.external.url`).
...
Fields: Base Glaze (relation to Glazes), Top Glaze (relation to Glazes, empty for single-glaze tiles), Clay (relation to Clays), Status (To-Do, Base Glazed, Fully Glazed, Finished), Layers (text, format `Base (CODE Name): N` then newline `Top (CODE Name): N` then notes), Name (`CODE Name → CODE Name (Clay)` for layered, or `CODE Name (Clay)` for single — the clay is in the title so a combo is easy to pick when adding it as a relation). About 23 rows are currently Finished, all on WH8. Resolve a tile's base and top strictly from the Base/Top relations (relation page id to glaze code); a row with no resolvable Base relation is skipped. Every combo row now carries its Base/Top relations, so Layers and Name are documentation only and are not parsed. Resolve clay via the Clay relation to clay name. For finished tiles, also collect photo urls so the detail panel can show the actual fired tile: read every Files and media property and keep the image-looking urls (auto-detect), or pin a single property by name via env `TILES_PHOTO_PROP`. Photos hang off the same (base, top, clay) key as the blackout and render in the detail panel for finished bands only. (The Notion database is titled "Glaze Combos"; each row is a combo and a Finished one is a finished tile. The `Index` number is the owner's per-combo key and the app does not consume it.)

### Throws (finished-piece overlay)
The Throws database (env `THROWS_DB_ID`) tracks thrown work. A throw can name the combo it used via a `Glaze Combos` relation (to the Glaze Combos rows above), carry a `Clay` relation, and hold a `Photos` files property where the owner uploads stage-by-stage shots — the latest image is the finished, glazed piece. Build a combo index mapping every Glaze Combos row id (any status, so untiled combos resolve too) to its {base, top} codes via the same Base/Top relation resolution as tiles. A throw becomes a "finished piece" when its Status is Finished or Fired, its combo resolves, it has a Clay, and it has at least one image; emit `{base, top, clay, photo}` using the latest image. Pieces hang off the same (base, top, clay) key. A band with a piece but no finished tile renders dark grey `#5b554c` (between the finished black and skip grey) and a finished tile outranks a piece. The detail panel shows piece photos in their own "Finished pieces" section, and pieces count toward the progress bar as bands made.
...
Two clay-independent corner tags on the cell: run risk is a top-right triangle `#7A4D0E`; decorative-only is a top-left triangle `#6B5FD6`. Single-tile cells get an inset ring (`box-shadow: inset 0 0 0 1.5px #6f685c`). Suppress both tags when every band in the cell is Skip. A band turns black `#17130d` when its tile is finished, or dark grey `#5b554c` when a thrown piece exists for that combo and clay but no tile does. Hover or tap a cell to show a detail panel (title, the run/decorative tags, a one-line recommendation with suggested coats, photos of any finished tiles and finished pieces in the cell, and one line per clay). Clicking a cell pins it so the panel stays put when the pointer leaves; clicking it again or the close button unpins. Include a header with a "live from Notion" timestamp, a legend, a progress bar, and a small glaze key grouped by product line.
